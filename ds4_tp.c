...
/* v11 over upstream v10: hello's pad word became odl_ok (OdinLink
 * negotiation) and the fork's mirrored-sync frames moved past GLM_MTP. */
...
int ds4_tp_big_gate_exchange(ds4_tp *tp, uint32_t layer, uint64_t seq,
     ...
     * the odl path validate it); upstream dropped it. */
